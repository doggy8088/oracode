# `oracode` 開發計畫 (Database as Code CLI for Oracle)

## 核心設計理念

1. **純淨的 DDL (Clean DDL)：** 徹底移除環境綁定參數 (Tablespace, Storage) 與多餘雜訊 (Editionable, 無效空白)，確保產出的 DDL 具有絕對的格式穩定性。
2. **Git-Friendly：** 單一物件單一檔案，目錄結構清晰，讓 `git diff` 只顯示真實的業務邏輯變更。
3. **安裝零摩擦 (Zero-Friction Install)：** 透過 `cargo-dist` 產生跨平台單一執行檔，並發布至 npm，開發者只需 `npm i -g oracode` 即可使用。
4. **高效能 (High Performance)：** 使用 Rust 實作，利用併發 (Concurrency) 處理大量物件的高速匯出。

---

## 階段開發藍圖

### Phase 1: 專案基礎建設與 CI/CD 管線建立 (Foundation & Pipeline)
...
* 設定安裝程式 (Installers) 包含 npm package 與 shell script/PowerShell。


* **1.3 GitHub Actions Workflow (CI/CD)：**
* **CI Pipeline (`ci.yml`)：** 處理 PR 與 Push 事件，包含 `cargo fmt --check`, `cargo clippy`, 與 `cargo test`。
* **CD Pipeline (`release.yml` by cargo-dist)：** 攔截 Git Tag (例如 `v1.0.0`) 事件，自動跨平台編譯、建立 GitHub Release、上傳二進位檔，並自動將包裝好的工具推送到 npm registry。



### Phase 2: 核心功能實作 - Oracle 資料庫互動 (DB Interaction)
...
### Phase 3: 核心功能實作 - DDL 淨化與格式穩定化 (Sanitization)
...
### Phase 4: 系統 I/O 與併發處理 (Concurrency & I/O)
...
### Phase 5: 文件撰寫與首發準備 (Documentation & Launch)
...
* **5.2 Release v0.1.0：**
* 打上 Git Tag 觸發 Phase 1 建立好的 CD Pipeline，檢視 GitHub Release 與 npm 發布是否成功。
